...
useEffect(()=>{
    const fetchData = async () => {
        ...
        const sortedJournalData = response.data.data.journals.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
  ...
  const fetchData = async () => {
   try {
    ...
    //    setGratitudeData(response.data.data.gratitudeData);
       ...
       const sortedGratitudeData = response.data.data.gratitudeData.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
...
<Text style={styles.heading}>My Journal</Text>
            ...
            <AntDesign name='search1' color='#fff' size={18} style={{marginLeft:8}}/>
...
<Text style={styles.tittle}>The most recent entries are included in the{'\n'}"Recent Entries" Section. Older entries are{'\n'} archived, and can be accessed under{'\n'} "Previous Entries"</Text>
...
style={styles.linear}
>
<Text style={{textAlign:"center", color:"#FFFFFF", fontFamily:"Now-Medium-BF6542f7137648d", fontSize:18}}>Previous Entries</Text>
</LinearGradient>
</TouchableOpacity>


<Text style={[styles.heading, {marginTop:28}]}>Recent Entries</Text>
...
{gratitudeData.slice(0,5).map((data, index) => {
    ...
    return(
        ...
        <TouchableOpacity style={styles.dataBtn} onPress={()=> onClick({type:"Gratitude", dataId: dataId})}>

        <View key={index} >
        <Text style={styles.currentDate}> {formattedDate}</Text>
        <Text style={[styles.dataBtnHeading, ]}>Gratitude</Text>
        <View style={{flexDirection:"row", alignItems:"center", justifyContent:"space-between",marginTop:2,}}>
        <Text style={styles.currentData}>{data.gratitudeInput1}</Text>
