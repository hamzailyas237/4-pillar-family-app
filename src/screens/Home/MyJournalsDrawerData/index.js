...
const MyJournalsDrawerData = ({route}) => {
    ...
    useEffect(()=>{
        ...
        useEffect(() => {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`https://4-pillar-backend.vercel.app/api/v1/journal/getJournal/${id}`);
                    const data = response.data.data.journals.find(item => item._id === dataId); 
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://4-pillar-backend.vercel.app/api/v1/gratitude/getGratitude/${id}`);
              
                const data = response.data.data.gratitudeData.find(item => item._id === dataId); // Find the specific entry by dataId
...
<TouchableOpacity  onPress={() => navigation.navigate("MyJournalsDrawerEdit" , { datetime: userData[0]?.createdAt })}>
<Feather name='edit' color='#fff' size={26} style={{marginRight:32}}/>
</TouchableOpacity> 
</View>
...
{journalData.slice(0, 1).map((data, index) => {
    console.log("🚀 ~ {journalData.slice ~ journalData:", journalData)
    const createdAtDate = new Date(data.createdAt);
    // Format the date to a more readable string
    const formattedDate = `${createdAtDate.toLocaleDateString()} ${createdAtDate.toLocaleTimeString()}`;
...
<Text style={styles.timeHeading}>Entry:1</Text>
    ...
    <Text style={[styles.timeHeading, {marginTop:16}]}>Entry:1</Text>
    <Text style={styles.time}>{data.reFrame.input1}</Text>
    <Text style={[styles.timeHeading, {marginTop:16}]}>Entry:2</Text>
    <Text style={styles.time}>{data.reFrame.input2}</Text>
    <Text style={[styles.timeHeading, {marginTop:16}]}>Entry:3</Text>
    <Text style={styles.time}>{data.reFrame.input3}</Text>
    <Text style={[styles.timeHeading, {marginTop:16}]}>Entry:4</Text>
    <Text style={styles.time}>{data.reFrame.input4}</Text>
    <Text style={[styles.timeHeading, {marginTop:16}]}>Entry:5</Text>
    <Text style={styles.time}>{data.reFrame.input5}</Text>
    </>
